...
7. Rewrite the Professional Summary, injecting JD keywords + the exit-narrative bridge ("Built and sold a business. Now applying systems thinking to [JD domain].")
...
12. Render the full HTML from the template + personalized content
...
| `{{PHONE}}` | (from profile.yml — include with its separator only when `profile.yml` has a non-empty `phone` value; otherwise omit both `<span>` and `<span class="separator">`) |
...
- **"Canva CV (visual, design-preserving)"** — the new flow below

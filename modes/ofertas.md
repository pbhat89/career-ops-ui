...
For each offer: score on each dimension, then a weighted total score.
Final ranking + recommendation with time-to-offer considerations.

Ask the user for the offers if they aren't in context. Input can be text, URLs, or references to offers already evaluated in the tracker.
